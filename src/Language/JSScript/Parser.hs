...
expr = exprEqual
  where
    ...
    exprIndex = ExprIndex <$> (exprLit <|> exprParens <|> try exprFuncCall <|> exprVar) <*> (symbol "[" *> expr <* symbol "]")
    exprSimple = exprLit <|> exprParens <|> try exprIndex <|> try exprFuncCall <|> try exprVar
