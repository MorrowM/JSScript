...
lit :: Parser Lit
lit = (LitText <$> litText) <|> litBool <|> try (LitDouble <$> litDouble) <|> (LitInt . fromInteger <$> litInt)
  where
...
expr =  exprEqual
  where
    ...
    exprSimple = exprLit <|> exprVar <|> exprFuncCall <|> exprParens
...
stmt = block <|> declare <|> try ifstmt <|> while <|> breakstmt <|> func <|> returnstmt <|> try assign <|> funcCall
  where
    ...
    returnstmt = StmtReturn <$> (symbol "return" *> expr)
